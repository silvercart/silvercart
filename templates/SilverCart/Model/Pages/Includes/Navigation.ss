            ...
            <% if not $EditableShoppingCart %>
                    <div class="btn-group cart-preview pull-right">
                        <a class="btn dropdown-toggle" data-toggle="dropdown" href="#">
                            <i class="icon-shopping-cart"></i> <% if $CurrentMember %><% with $CurrentMember %>{$ShoppingCart.getQuantity}<% end_with %><% else %>0<% end_if %> <%t SilverCart\Model\Product\Product.PLURALNAME 'Products' %>
