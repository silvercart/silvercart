<table style="border-top: 3px solid #dddddd;">
    <tbody>
        ...
        <tr>
            <td style="text-align: right; border: none; border-top: 3px solid #dddddd; padding: 1.0em 0.5em 0.5em;" colspan="2"><%t SilverCart\Model\Pages\Page.VALUE_OF_GOODS 'Value of goods' %></td>
        ...
        <% if $HasChargePositionsForProduct %>
            ...
            <% with $getTaxableAmountGrossWithoutFees(true,false) %>
                <tr>
                    <td style="text-align: right; border: none; padding: 1.0em 0.5em 0.5em;" colspan="2"><%t SilverCart\Model\Pages\Page.SUBTOTAL 'Subtotal' %></td>
            ...
            <% loop $getTaxRatesWithoutFees(true,false) %>
                <tr>
                    <td style="text-align: right; border: none; padding: 1.0em 0.5em 0.5em;" colspan="2"><%t SilverCart\Model\Pages\Page.INCLUDED_VAT 'included VAT' %> ({$Rate}%)</td>
                    <td style="text-align: right; border: none; padding: 1.0em 0.5em 0.5em;">{$Amount.Nice}</td>
                </tr>
            <% end_loop %>
        <% end_if %>
        ...
        <% if $OrderChargePositionsTotal %>
            ...
            <% loop $getTaxRatesWithFees(true,false) %>
                <tr>
                    <td style="text-align: right; border: none; padding: 1.0em 0.5em 0.5em; color: #999;" colspan="2"><%t SilverCart\Model\Pages\Page.INCLUDED_VAT 'included VAT' %> ({$Rate}%)</td>
                    <td style="text-align: right; border: none; padding: 1.0em 0.5em 0.5em; color: #999;">{$Amount.Nice}</td>
                </tr>
            <% end_loop %>
        ...
        <% if $HasIncludedInTotalPositions %>
            <% loop $OrderIncludedInTotalPositions %>
                <tr>
                    <td class="text-right d-none d-sm-table-cell" colspan="2">{$Title.RAW}</td>
