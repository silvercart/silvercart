...
<% end_loop %>
    ...
    <% end_with %>
    <% loop $Actions %>
        <button title="{$Up.Product.Title} <%t SilverCart\Model\Product\Product.ADD_TO_CART 'add Cart' %>" class="btn btn-small btn-primary" data-placement="top" data-toggle="tooltip"><i class="icon-shopping-cart"></i></button>
    <% end_loop %>
    </div>
<% if $Product.isInCart %>
