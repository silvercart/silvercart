...
<div class="row ProductGroupHolderTile">
    ...
    <% loop $Elements %>
        <li class="span4 clearfix">
            ...
            <div class="thumbnail">
                <% if $groupPicture %>
                    <a href="{$Link}" title="<% sprintf(_t('SilvercartPage.SHOW_DETAILS_FOR','details'),$Image.Title) %>">{$groupPicture.SetSize(290,290)}</a>
                <% end_if %>
            </div>
            <div class="sc-product-shortinfo">                       
                <div class="product-info"><p>{$Content.LimitWordCount(12)}</p></div>
